...
class Cart extends Connection
{
    ...
    public function deleteProduct($id){
        $sql = "DELETE FROM product_cart WHERE SKU = $id AND (product_cart.cartId = '$_SESSION[cartId]')";
    ...
    public function totalPrice(){
        $sql = "SELECT SUM(prodCartNum*prodPriceSale) AS total FROM cart
        JOIN product_cart ON cart.cartId = product_cart.cartId
        JOIN product ON product.SKU = product_cart.SKU
        WHERE product_cart.cartId = '$_SESSION[cartId]'";
        // $sql = "SELECT SUM(
        //     CASE 
    ...
    public function insertOrder($data, $listProd){
        $sql = "INSERT INTO orders VALUES ('$data[orderId]', '$_SESSION[cusId]', '1', '', '$data[note]');";
